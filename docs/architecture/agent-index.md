...
- `server/src/db/index.ts` — `getDb()`
  - SQLite WAL 模式, review_records + settings 两张表, idx_review_records_cache 索引
...
### 全局导航
- `client/src/components/NavBar.tsx` — 全局导航栏 (tab 风格路由切换 Grid/Review/Random, 内含 GridControls, ReviewControls)
...
### API 客户端层
- `client/src/api/index.ts` — `api` 对象 (所有后端调用), 类型定义 (PhotoGroup, SubfolderInfo, ExifData, Stats, BrowseResult, ScanResult)
...
### 路由
- `client/src/App.tsx` — 3 条路由: `/` `/grid` `/review` `/random`，AppProvider 包裹
...
| 添加新页面 | App.tsx | 参考 GridPage.tsx 或 ReviewPage.tsx, api/index.ts |
